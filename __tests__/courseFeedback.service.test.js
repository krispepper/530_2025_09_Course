// Jest suite validating the course feedback service contract and edge cases.
jest.mock("../src/db", () => ({
    getDB: jest.fn()
}));
...
describe("courseFeedbackService", () => {
    ...
    it("lists feedback using provided filters", async () => {
        ...
        const rows = await listFeedback({ courseId: " CS101 ", studentId: " s-001 " });
        expect(collection.find).toHaveBeenCalledWith({ courseId: "CS101", studentId: "s-001" });
    ...
    it("rejects non-string filters", async () => {
        await expect(listFeedback({ courseId: 123 })).rejects.toThrow("courseId must be a string");
        await expect(listFeedback({ studentId: { value: "abc" } })).rejects.toThrow("studentId must be a string");
    });
    ...
    it("builds a trimmed summary pipeline when courseId is provided", async () => {
        const aggregateCursor = {
            toArray: jest.fn().mockResolvedValue([])
        };
        collection.aggregate = jest.fn(() => aggregateCursor);

        await getFeedbackSummary(" CS101 ");

        expect(collection.aggregate).toHaveBeenCalledWith([
            { $match: { courseId: "CS101" } },
            { $sort: { createdAt: -1 } },
            {
                $group: {
                    _id: "$courseId",
        ...
        expect(aggregateCursor.toArray).toHaveBeenCalled();
    });

    it("omits the match stage when courseId is blank", async () => {
        const aggregateCursor = {
            toArray: jest.fn().mockResolvedValue([])
        };
        collection.aggregate = jest.fn(() => aggregateCursor);
